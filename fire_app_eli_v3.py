...
/* Buttons */
.stButton>button {{
    background: var(--primary); color: white; border-radius: 8px; border: none;
}}
...
.header-title h1 {{ margin-bottom:0.25rem; }}
@media (max-width: 640px) {{
  .header-wrap {{ flex-direction: row; gap: 10px; }}
...
with cols[0]:
    try:
        logo = Image.open("edelweiss_logo_cropped.png")
    except Exception:
        # fallback: try to crop from source path if present
        try:
            from PIL import Image
            src = Image.open("edelweiss theme.jpg")
            w, h = src.size
            box = (int(0.06*w), int(0.10*h), int(0.86*w), int(0.52*h))
            logo = src.crop(box)
        except Exception:
            logo = None
    if logo is not None:
...
with cols[1]:
    st.markdown('<div class="header-wrap"><div class="header-title"><h1>FIRE Calculator — Financial Independence, Retire Early</h1><p class="smallnote">Edelweiss palette • Simple inputs • Lean/Barista/Fat • SWR • Coast-FIRE • Mobile-friendly</p></div></div>', unsafe_allow_html=True)

with st.sidebar:
    st.header("Quick Start (minimal)")
    ...
    if sip_mode == "% of income":
        sip_pct = st.slider("SIP as % of income", 0, 80, 30, 1,
...
with left:
    ...
    st.metric("Required corpus (₹)", rupee_indian(required_corpus))
    st.metric("Projected corpus at target age", rupee_indian(projected_corpus_at_target))
    if projected_corpus_at_target >= required_corpus:
        ...
        st.write(f"Shortfall at {target_age}: **{rupee_indian(required_corpus - projected_corpus_at_target)}**")

    st.subheader("Earliest FI (with the above plan)")
    if age_reached <= 80:
        st.success(f"FI reached by **age {age_reached:.1f}** with corpus ≈ **{rupee_indian(corpus_when_reached)}**.")
    else:
        st.warning("FI not reached by age 80 under current assumptions.")

    st.subheader("Coast-FIRE")
    if coast_age is not None:
        st.info(f"If you stop investing now, your current corpus would naturally reach FI by **age {coast_age:.1f}**.")
    else:
        st.info("Coast-FIRE not achievable by age 80 with only current corpus.")
...
with right:
    ...
    with st.expander("Snapshots (every ~6 months)"):
        if not traj_df.empty:
            ...
            st.caption("Will appear after you set inputs.")
...
st.caption("Edelweiss-themed • Left logo header • Currency axes in ₹L/₹Cr • Mobile-friendly layout • No flame icon")
