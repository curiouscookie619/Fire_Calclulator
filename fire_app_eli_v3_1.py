...
/* Title scaled to not overpower logo */
.header-title h1 {{
    margin: 0;
    font-size: 1.6rem;
    line-height: 1.2;
...
with c1:
    try:
        ...
        pass
with c2:
    st.markdown('<div class="header-wrap"><div class="header-title"><h1>FIRE Calculator — Financial Independence, Retire Early</h1><div class="smallnote">Edelweiss palette • Simple inputs • Lean/Barista/Fat • SWR • Coast-FIRE</div></div></div>', unsafe_allow_html=True)
...
# -----------------------------
# Output UI — intuitive view
# -----------------------------
# At-a-glance cards
g1, g2, g3 = st.columns([1,1,1])
with g1:
    st.markdown('<div class="card"><h3>Required corpus</h3><div class="value">' + rupee_indian(required_corpus) + '</div></div>', unsafe_allow_html=True)
with g2:
    st.markdown('<div class="card"><h3>Projected @ target age</h3><div class="value">' + rupee_indian(projected_corpus_at_target) + '</div></div>', unsafe_allow_html=True)
with g3:
    if age_reached <= 80:
        st.markdown(f'<div class="card"><h3>Earliest FI age</h3><div class="value">{age_reached:.1f} yrs</div></div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="card"><h3>Earliest FI age</h3><div class="value">Not by 80</div></div>', unsafe_allow_html=True)

# Progress toward target corpus
...
st.progress(ratio_clamped, text=f"{ratio*100:.1f}% of target")
...
st.caption("Edelweiss-themed • Left logo header • Currency axes in ₹L/₹Cr • Intuitive 'At-a-glance' + progress • Mobile-friendly")
